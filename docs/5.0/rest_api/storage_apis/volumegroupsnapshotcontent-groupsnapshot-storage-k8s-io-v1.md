...
| `spec`       | `object`                                                                             | Spec defines properties of a VolumeGroupSnapshotContent created by the underlying storage system. Required.                                                                                                                                                                                          |
| `status`     | `object`                                                                             | status represents the current information of a group snapshot.                                                                                                                                                                                                                                       |
...
| `volumeGroupSnapshotClassName` | `string` | VolumeGroupSnapshotClassName is the name of the VolumeGroupSnapshotClass from which this group snapshot was (or will be) created. Note that after provisioning, the VolumeGroupSnapshotClass may be deleted or recreated with different set of values, and as such, should not be referenced post-snapshot creation. For dynamic provisioning, this field must be set. This field may be unset for pre-provisioned snapshots.                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `volumeGroupSnapshotRef`       | `object` | VolumeGroupSnapshotRef specifies the VolumeGroupSnapshot object to which this VolumeGroupSnapshotContent object is bound. VolumeGroupSnapshot.Spec.VolumeGroupSnapshotContentName field must reference to this VolumeGroupSnapshotContent’s name for the bidirectional binding to be valid. For a pre-existing VolumeGroupSnapshotContent object, name and namespace of the VolumeGroupSnapshot object MUST be provided for binding to happen. This field is immutable after creation. Required.                                                                                                                                                                                                                                                                                                                                              |
...
| Property               | Type             | Description                                                                                                                                                                                                                                                                              |
|------------------------|------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `groupSnapshotHandles` | `object`         | GroupSnapshotHandles specifies the CSI "group_snapshot_id" of a pre-existing group snapshot and a list of CSI "snapshot_id" of pre-existing snapshots on the underlying storage system for which a Kubernetes object representation was (or should be) created. This field is immutable. |
| `volumeHandles`        | `array (string)` | VolumeHandles is a list of volume handles on the backend to be snapshotted together. It is specified for dynamic provisioning of the VolumeGroupSnapshot. This field is immutable.                                                                                                       |
...
| Property                    | Type             | Description                                                                                                                                                                                                                                   |
|-----------------------------|------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `volumeGroupSnapshotHandle` | `string`         | VolumeGroupSnapshotHandle specifies the CSI "group_snapshot_id" of a pre-existing group snapshot on the underlying storage system for which a Kubernetes object representation was (or should be) created. This field is immutable. Required. |
| `volumeSnapshotHandles`     | `array (string)` | VolumeSnapshotHandles is a list of CSI "snapshot_id" of pre-existing snapshots on the underlying storage system for which Kubernetes objects representation were (or should be) created. This field is immutable. Required.                   |
...
| Property                    | Type      | Description                                                                                                                                                                                                                                                                                                                           |
|-----------------------------|-----------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `creationTime`              | `string`  | CreationTime is the timestamp when the point-in-time group snapshot is taken by the underlying storage system. If not specified, it indicates the creation time is unknown. If not specified, it means the readiness of a group snapshot is unknown. This field is the source for the CreationTime field in VolumeGroupSnapshotStatus |
| `error`                     | `object`  | Error is the last observed error during group snapshot creation, if any. Upon success after retry, this error field will be cleared.                                                                                                                                                                                                  |
| `readyToUse`                | `boolean` | ReadyToUse indicates if all the individual snapshots in the group are ready to be used to restore a group of volumes. ReadyToUse becomes true when ReadyToUse of all individual snapshots become true.                                                                                                                                |
| `volumeGroupSnapshotHandle` | `string`  | VolumeGroupSnapshotHandle is a unique id returned by the CSI driver to identify the VolumeGroupSnapshot on the storage system. If a storage system does not provide such an id, the CSI driver can choose to return the VolumeGroupSnapshot name.                                                                                     |
| `volumeSnapshotInfoList`    | `array`   | VolumeSnapshotInfoList is a list of snapshot information returned by the CSI driver to identify snapshots on the storage system.                                                                                                                                                                                                      |
| `volumeSnapshotInfoList[]`  | `object`  | VolumeSnapshotInfo contains information for a snapshot                                                                                                                                                                                                                                                                                |
...
| Property         | Type      | Description                                                                                              |
|------------------|-----------|----------------------------------------------------------------------------------------------------------|
| `creationTime`   | `integer` | creationTime is the timestamp when the point-in-time snapshot is taken by the underlying storage system. |
| `readyToUse`     | `boolean` | ReadyToUse indicates if the snapshot is ready to be used to restore a volume.                            |
| `restoreSize`    | `integer` | RestoreSize represents the minimum size of volume required to create a volume from this snapshot.        |
| `snapshotHandle` | `string`  | SnapshotHandle is the CSI "snapshot_id" of this snapshot on the underlying storage system.               |
| `volumeHandle`   | `string`  | VolumeHandle specifies the CSI "volume_id" of the volume from which this snapshot was taken from.        |

# API endpoints

The following API endpoints are available:

- `/apis/groupsnapshot.storage.k8s.io/v1/volumegroupsnapshotcontents`
...
- `/apis/groupsnapshot.storage.k8s.io/v1/volumegroupsnapshotcontents/{name}`
...
- `/apis/groupsnapshot.storage.k8s.io/v1/volumegroupsnapshotcontents/{name}/status`
...
| HTTP code          | Reponse body                                                                                                                   |
|--------------------|--------------------------------------------------------------------------------------------------------------------------------|
| 200 - OK           | [`VolumeGroupSnapshotContentList`](../objects/index.xml#io-k8s-storage-groupsnapshot-v1-VolumeGroupSnapshotContentList) schema |
| 401 - Unauthorized | Empty                                                                                                                          |
...
| Parameter | Type                                                                                                                                                                             | Description |
|-----------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------|
| `body`    | [`VolumeGroupSnapshotContent`](../storage_apis/volumegroupsnapshotcontent-groupsnapshot-storage-k8s-io-v1.xml#volumegroupsnapshotcontent-groupsnapshot-storage-k8s-io-v1) schema |             |

Body parameters

| HTTP code          | Reponse body                                                                                                                                                                     |
|--------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 200 - OK           | [`VolumeGroupSnapshotContent`](../storage_apis/volumegroupsnapshotcontent-groupsnapshot-storage-k8s-io-v1.xml#volumegroupsnapshotcontent-groupsnapshot-storage-k8s-io-v1) schema |
| 201 - Created      | [`VolumeGroupSnapshotContent`](../storage_apis/volumegroupsnapshotcontent-groupsnapshot-storage-k8s-io-v1.xml#volumegroupsnapshotcontent-groupsnapshot-storage-k8s-io-v1) schema |
| 202 - Accepted     | [`VolumeGroupSnapshotContent`](../storage_apis/volumegroupsnapshotcontent-groupsnapshot-storage-k8s-io-v1.xml#volumegroupsnapshotcontent-groupsnapshot-storage-k8s-io-v1) schema |
| 401 - Unauthorized | Empty                                                                                                                                                                            |
...
| Parameter | Type     | Description                            |
|-----------|----------|----------------------------------------|
| `name`    | `string` | name of the VolumeGroupSnapshotContent |
