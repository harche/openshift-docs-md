...
- The Operator subscription must opt in to TrustManager by setting `UNSUPPORTED_ADDON_FEATURES=TrustManager=true`.
  ...
  For more information, see [Enabling the TrustManager Operand](../../security/cert_manager_operator/cert-manager-trust-manager.xml#cert-manager-trust-manager-install_cert-manager-trust-manager).
...
- `cert-manager-cainjector`: metrics endpoint TLS flags only.
  ...
  This feature is gated by the `TLSAdherence` feature gate. To use this feature, you must enable the `TechPreviewNoUpgrade` feature set. For more information, see [Understanding feature gates](../../nodes/clusters/nodes-cluster-enabling-features.xml#nodes-cluster-enabling-features-about_nodes-cluster-enabling).
