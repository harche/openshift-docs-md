...
[OpenShift Container Platform events](../../nodes/clusters/nodes-containers-events.xml#nodes-containers-events) are records of important life-cycle information and are useful for monitoring and troubleshooting virtual machine, namespace, and resource issues.
...
- VM events: Navigate to the **Events** tab of the **VirtualMachine details** page in the web console.
  ...
  See the [list of events](../../nodes/clusters/nodes-containers-events.xml#nodes-containers-events-list_nodes-containers-events) for details about specific events.
...
You can view logs for OpenShift Virtualization pods by using the web console or the CLI. You can also view [aggregated logs](../../virt/support/virt-troubleshooting.xml#virt-viewing-logs-loki_virt-troubleshooting) by using the LokiStack in the web console.
...
Even if the guest VM has no network, you can access it using its VNC or serial console, as documented in [Connecting to virtual machine consoles](../../virt/managing_vms/virt-accessing-vm-consoles.xml#virt-accessing-vm-consoles)
