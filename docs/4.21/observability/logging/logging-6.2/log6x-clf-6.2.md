...
1.  Create or edit a YAML file that defines the `ClusterLogForwarder` CR object:
    ...
    - Optional: Set the severity level for the message. For more information, see [The Syslog Protocol](https://datatracker.ietf.org/doc/html/rfc5424#section-6.2.1).
...
The `ClusterLogForwarder` custom resource (CR) uses the same format as the standard [Kubernetes audit policy](https://kubernetes.io/docs/tasks/debug/debug-cluster/audit/#audit-policy), while providing the following additional functions:
